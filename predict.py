...
def predict(model, loss_fn, test_loader, params, plot_num, sample=True):
    ...
    with torch.no_grad():
      ...
      for i, (test_batch, id_batch, v, labels) in enumerate(tqdm(test_loader)):
          ...
          if sample:
              ...
              raw_metrics = utils.update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, labels, params.test_.predict_start, samples, relative = params.relative_metrics)
...
if __name__ == '__main__':
    ...
    test_metrics = predict(model, loss_fn, test_loader, params, -1, params.sampling)
