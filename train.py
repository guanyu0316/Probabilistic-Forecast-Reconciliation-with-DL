...
def train(model: nn.Module,
    ...
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        ...
        for t in range(params.train_window):
            ...
            loss += loss_fn(mu, sigma, labels_batch[t],params.lam,params.dataset)
...
def train_and_evaluate(model: nn.Module,
    ...
    for epoch in range(params.num_epochs):
        ...
        test_metrics,plot_metrics = evaluate(model, loss_fn, test_loader, params, epoch, args.dataset, sample=args.sampling)
        if epoch==params.num_epochs-1:
            np.save(f'{params.model_dir}/eval_metrics.npy', plot_metrics)
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_set, batch_size=params.batch_size, shuffle=False, num_workers=4)
